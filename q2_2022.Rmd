...
fsc %>%  
  ...
  kable(caption = "2022 humanitarian beneficiaries", 
...
fsc %>%  
  ...
  adorn_totals("row") %>% 
  mutate(`%_increase` = round((Total_Jun22 - Q1) /Q1 * 100, digits = 2)) %>%
  kable(caption = "2022 development beneficiaries", 
        format.args = list(big.mark = ","), 
        format = "html", 
        table.attr = "style='width:60%;'") %>% 
  kable_classic_2("striped")
...
fsc %>% 
  ...
  arrange(desc(Total)) %>% 
  kable(caption = "Beneficiaries by activity, as  of 30 June 2022", format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped")
...
Food distributions overwhelmingly target persons in host and local communities, this group forms `r ben_type_list %>% filter(beneficiary_type == "Host/local Community") %>% pull(pc)`% of all beneficiaries of food distributions or `r ben_type_list %>% filter(beneficiary_type == "Host/local Community") %>% pull(beneficiaries) %>% format(big.mark = ",")` persons. However, as will be seen in section 5, the food insecurity status of beneficiaries is often reported incorrectly. 
...
fsc %>% 
  ...
  labs(y = "Number of locations", 
       ...
       title = "Project locations and beneficiaries, faceted by number of activities") +
  theme(legend.position = "none",
          strip.text = element_text(size = 10, face = "bold"),
          strip.background = element_rect(fill = "#212121"))
```
...
fsc %>% 
  ...
  kable(caption = "Number of partners by location, as of 30 June 2022", 
        format.args = list(big.mark = ","), 
        format = "html", 
...
fsc %>% 
  ...
  kable(caption = "Percentage of beneficiaries reached by activity and delivery modality", 
        format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped") %>% 
  footnote(general = "Beneficiary totals are as of 30 June 2022", 
           general_title = "")
```
...
food_dist_range <- fsc %>% 
  ...
  scale_x_continuous(breaks = c(0, 3, 10, 20, 30, 50), labels = dollar_format(accuracy = 1), 
                     trans = "log10") +
...
fsc %>% 
  ...
  labs(x = "", y = "Number of partners", 
       colour = "Parnter\ntype",
       title = "Number of partners reporting by month, as of Q2 2022")
...
fsc %>%
  ...
  head(15) %>% 
  kable(caption = "Top implementing partners by beneficiaries reached in 2022, as of 30 June 2022", 
        format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped") 
```
...
fsc %>% 
  ...
  kable(caption = "Top donors by beneficiaries reached", 
        format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped", full_width = FALSE) %>% 
  footnote("Donors starting with 'org_xxxx' are partners using their own organisational funds", 
           general_title = "",
           threeparttable = TRUE)
...
fsc %>% 
  ...
  kable(caption = "Missing food insecurity data of beneficiaries, as of 30 June 2022", 
        format.args = list(big.mark = ","), 
        format = "html", 
...
fsc %>% 
  ...
  mutate(`%_beneficiaries` = round(beneficiaries / sum(beneficiaries) * 100, digits = 2)) %>% 
  arrange(desc(beneficiaries)) %>% 
  kable(caption = "Evidence of food insecurity status, as of 30 June 2022", 
        format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped", full_width = FALSE)
```
...
ggplotly(ben_target, tooltip = c("text"), width = 820, height = 500) %>% 
  config(displayModeBar = FALSE) %>% 
  layout(title = list(text = paste0("Beneficiaries vs target by township, as of Q2 2022",
                                    "<br>",
                                    "<sup>",
...
```{r plotly-tsp-comparison-reached-target-2021}
...
ben_target <- fsc %>% 
  ...
  labs(x = "Targeted population", y = "Beneficiaries", 
       title = "Beneficiaries vs target by township, **Q1** 2022",
       subtitle = "The red line is 100% of target") + 
  theme(legend.title = element_text(size = 7))
...
fsc %>% 
  ...
  mutate(pc = round(beneficiaries / target * 100, digits = 2)) %>% 
  select(state, township, Q1 = q1, Q2 = q2, 
...
fsc %>% 
  ...
  theme(legend.text = element_text(size = 10),
        ...
        legend.key.size = unit(0.7, 'cm'), 
        plot.background = element_rect(fill = "white", colour = "white")) +
  labs(title = "Beneficiaries Total 2022 as of Q2 2022, by township",
       subtitle = "townships in grey do not have any partners present", 
       fill = "Beneficiaries") +
pin %>% 
