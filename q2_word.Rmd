...
Overall, `r hd_list %>% filter(humanitarian_or_development == "Humanitarian") %>% pull(pc)`% of the food security cluster's beneficiaries were from humanitarian activities. 
...
fsc %>%  
  ...
  select(SO, Q1, Q2, 
         Total, `total%` = total_pc) %>% 
  adorn_totals("row") %>% 
  pander(caption = "2022 beneficiaries by strategic objective", style = "grid")
  # kable(caption = "2022 Q1 & Q2 humanitarian beneficiaries", 
...
A total of `r hd_list %>% filter(humanitarian_or_development == "Development") %>% pull(pc)`% beneficiaries were from development activities and actors. 
...
fsc %>%  
  filter(humanitarian_or_development == "Development") %>% 
  group_by(so, quarter) %>% 
  summarise(beneficiaries = sum(new_beneficiaries), .groups = "drop") %>% 
  pivot_wider(names_from = quarter, 
              values_from = beneficiaries, 
  ...
  select(SO, Q1, Q2, 
         Total, `total%` = total_pc) %>% 
  adorn_totals("row") %>%  
  pander(caption = "2022 Q1 & Q2 development beneficiaries", style = "grid")
  # kable(caption = "2022 Q1 & Q2 development beneficiaries", 
  #       format.args = list(big.mark = ","), 
  #       format = "html", 
  #       table.attr = "style='width:60%;'") %>% 
  # kable_classic_2("striped")
...
Food distributions overwhelmingly target moderately-food-insecure persons in host and local communities, this group forms `r ben_type_list %>% filter(beneficiary_type == "Host/local Community") %>% pull(pc)`% of all beneficiaries of food distributions. 
...
fsc %>% 
  ...
  pander(caption = "Food distributions by type food insecurity status and beneficiary type", 
         style = "grid")
 # kable(caption = "Food distributions by type food insecurity status and beneficiary type", 
 #       format.args = list(big.mark = ",")) %>% 
 # kable_classic_2("striped")
...
fsc %>% 
  ...
  pander(caption = "Beneficiaries by status of nutrition mainstreaming", 
         style = "grid")
...
Though new beneficiaries reached remained biased towards Yangon and Rakhine in Q2, figures were less skewed than they were in Q1. Overall `r state_list %>% filter(quarter == "q2" & state %in% c("Yangon", "Rakhine")) %>% summarise_at("pc", sum) %>% pull(pc)`% of beneficiaries in Q2 came from Yangon or Rakhine, whereas it was `r state_list %>% filter(quarter == "q1" & state %in% c("Yangon", "Rakhine")) %>% summarise_at("pc", sum) %>% pull(pc)`% in Q1. Kayah saw the largest quarter-to-quarter increase in number of beneficiaries. 
...
A total of `r distinct(fsc, admin3_pcode) %>% nrow()` townships have been reached across `r distinct(fsc, admin1_pcode) %>% nrow()` states/regions. 
...
fsc %>% 
  ...
  select(state, township, Q1 = q1, Q2 = q2, 
         Total = total, `%total` = pc_total) %>%
  pander(caption = "Top townships by beneficiaries reached", 
         style = "grid")
...
fsc %>% 
 ...
 # mutate(partners = ifelse(partners >= 3, ">=3", partners), 
 #        partners = fct_relevel(partners, 
 #                                 c("1", "2", ">=3")),
 #        partners = recode(partners, 
 #                            "1" = "1_activity", 
 #                            "2" = "2_partners", 
 #                            ">=3" = ">=3_partners"),) %>% 
  ggplot(aes(x = partners, fill = location_type)) + 
  geom_histogram(binwidth = 1) + 
  stat_bin(binwidth = 1, geom = "text", size = 3, 
           aes(label = ..count..), 
           position = position_stack(vjust = 0.9)) + 
  scale_fill_manual(values = c("#F1C40F", "#1ABC9C", "#EC7063")) +
  facet_wrap(~ location_type) + 
  labs(y = "Number of locations", 
       x = "Number of partners by location", 
       title = "Beneficiary frequencies by location, faceted by number of partners") +
  theme(legend.position = "none",
          strip.text = element_text(size = 10, face = "bold"),
          strip.background = element_rect(fill = "#212121"))
```
...
fsc %>%  
  ...
  summarise(beneficiaries = sum(beneficiaries)) %>% 
  arrange(desc(beneficiaries)) %>%
  pander(caption = "Activities conducted in villages with more than one partner", 
         style = "grid")
...
The first grey line below shows the the approval of the IERP in June 2021 and the red line shows the start of 2022. 
...
The percentage of beneficiaries reached by agriculture and livelihoods activities (crops, vegetable and seed kits; FFS and farmer training; IGA and small grants; livestock kits) declined in Q2 2022, compared to Q1. 
...
fsc %>% 
  ...
  ggplot(aes(x = beneficiaries, y = activity_red, fill = activity_red)) + 
  geom_col(position = position_dodge(width = .9)) + 
...
fsc %>% 
  ...
  pander(caption = "Percentage of beneficiaries reahced by activity and delivery modality", 
         style = "grid")
...
fsc %>%  
  ...
  sum_ben2(delivery_modality, beneficiary_type) %>% 
  ggplot(aes(x = delivery_modality, y = beneficiaries, fill = delivery_modality)) +
  geom_col() + 
  scale_y_continuous(labels = comma) + 
  facet_wrap(~beneficiary_type, scales = "free") + 
...
ggplotly(money_scatter, tooltip = c("text"), width = 820) %>% 
  layout(title = list(text = paste0("Average USD transfer value per person in 2022 (Q1 & Q2)")))
```
...
# food_dist_range <- 
  ...
  labs(x = "USD value per person", 
       y = "", 
       title = "Food distribution: range of USD values per person by state in 2022 (Q1 & Q2)",
       size = "", colour = "") 
...
ggplotly(food_dist_range, tooltip = c("text"), width = 820, height =  600) %>% 
  layout(title = list(text = paste0("Food distribution: range of USD values per person by state in 2022 (Q1 & Q2)",
                                    "<br>",
                                    "<sup>",
...
fsc %>% 
  ...
  mutate(beneficiary_type = fct_reorder(beneficiary_type, beneficiaries), 
         quarter = recode(quarter, 
                          "q1" = "Quarter_1", 
                          "q2" = "Quarter_2")) %>% 
  ggplot(aes(y = beneficiary_type, x = beneficiaries, fill = beneficiary_type)) + 
  geom_col() +
...
fsc %>% 
  ...
  group_by(activity) %>% 
  mutate(`%of_group` = round(beneficiaries / sum(beneficiaries) * 100, digits = 2)) %>%
  pander(caption = "Mismatch between food insecurity status and activity (Q1 & Q2 2022)", 
         style = "grid")
  # kable(caption = "Mismatch between food insecurity status and activity (Q1 & Q2 2022)", 
  #       format.args = list(big.mark = ","), 
  #       format = "html", 
...
fsc %>% 
  ...
  arrange(desc(beneficiaries)) %>% 
  pander(caption = "Evidence of food insecurity status in Q1 & Q2 2022", 
         style = "grid")
...
fsc %>% group_by(report_org_code) %>% 
  ...
  pander(caption = "Reporting organisations with the most implementing partners", 
         style = "grid")
  # kable(caption = "Reporting organisations with the most implementing partners", 
  #       format = "html", 
  #       table.attr = "style='width:45%;'") %>% 
  # kable_classic_2("striped") %>% 
  # footnote(general = "All others had 1 or 2 implementing partners", 
  #          general_title = "")
```
...
`r fsc %>% group_by(org_code) %>% summarise(townships = n_distinct(admin3_pcode)) %>% filter(townships > 5) %>% nrow()` partners (21% of the total) have a presence in more than 5 townships. `r fsc %>% group_by(org_code) %>% summarise(townships = n_distinct(admin3_pcode)) %>% filter(townships > 10) %>% nrow()` partners are present in more than 10 townships. 
...
Organisations such as 9693, 1206, 5440, 9566 and 6197 have implemented all or almost all their activities in 2022. 
...
fsc %>%
  ...
  labs(x = "Date", 
       ...
       subtitle = "Figures show cumulative unique beneficiaries; first red line is the IERP, second is the start of 2022") 
...
fsc %>%
  ...
  head(15) %>% 
  pander(caption = "Top implementing partners by beneficiaries reached in 2022 (Q1 & Q2)", 
         style = "grid")
...
fsc %>% 
  ...
  head(15) %>% 
  pander(caption = "Organisations supported and geographic reach by donor", 
         style = "grid")
  # kable(caption = "Organisations supported and geographic reach", 
  #       format.args = list(big.mark = ",")) %>% 
  # kable_classic_2("striped", full_width = FALSE) %>% 
  # footnote(general = "Only showing donors supporting more than one reporting partner.", 
  #          general_title = "")
...
fsc %>% 
  ...
  arrange(desc(donors)) %>% 
  pander(caption = "Number of donors by state", 
         style = "grid")
...
However, as shown by the table below, even if the majority of partners reported their donors, the omission of data from three key partners has resulted in the vast majority of reported beneficiaries not being associated with any donor. 
...
fsc %>% 
  ...
  head(15) %>% 
  pander(caption = "Top donors by beneficiaries reached", 
         style = "grid")
...
fsc %>% 
  ...
  pander(caption = "Reported beneficiaries with missing donor data", 
         style = "grid")
...
# ben_target <- 
  ...
  labs(x = "Targeted population", y = "Beneficiaries", 
       title = "Beneficiaries reached vs targeted population by township, Q1 & Q2 2022",
       subtitle = "The red line is 100% of target; size indicates number of beneficiaries") + 
...
ggplotly(ben_target, tooltip = c("text"), width = 820, height = 500) %>% 
  config(displayModeBar = FALSE) %>% 
  layout(title = list(text = paste0("Beneficiaries reached vs targeted population by township, Q1 & Q2 2022",
                                    "<br>",
                                    "<sup>",
